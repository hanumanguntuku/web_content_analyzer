...
async def test_gemini_failure_openai_success(monkeypatch, llm_service):
    ...
    with patch("requests.post", side_effect=Exception("Gemini fail")):
...
async def test_get_content_summary(monkeypatch, llm_service):
    with patch.object(llm_service, "_call_llm", return_value="summary") as mock_call:
        result = await llm_service.get_content_summary("text")
        assert result == "summary"

@pytest.mark.asyncio
async def test_get_sentiment_and_tone(monkeypatch, llm_service):
    with patch.object(llm_service, "_call_llm", return_value='{"sentiment_score": 0.5, "sentiment_label": "Positive", "detected_tones": ["Formal"]}') as mock_call:
...
async def test_get_seo_recommendations(monkeypatch, llm_service):
    with patch.object(llm_service, "_call_llm", return_value='{"overall_score": 90, "recommendations": ["Use more keywords"]}') as mock_call:
...
async def test_get_readability_and_accessibility(monkeypatch, llm_service):
    with patch.object(llm_service, "_call_llm", return_value='{"readability_score": 80, "accessibility_notes": ["Use simpler sentences"]}') as mock_call:
